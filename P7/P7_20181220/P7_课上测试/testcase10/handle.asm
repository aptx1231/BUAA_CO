...
.text 
	...
	ori	$8, $8, 0xffff
	j	slot_ov1
	add	$10, $8, $9	        # 测试延迟槽内 add 的上界溢�?
slot_ov1:
	lui     $t0,0x8000
	jal	slot_ov2
	addi	$10, $8, -1     # 测试延迟槽内 addi 的下界溢�?
slot_ov2:
	ori	$t1, $0, 0x0ba0
	lui	$8, 0x8000
	lui	$9, 0x1000
	j	slot_ov3
	sub	$10, $8, $9			# 测试延迟槽内 sub 的下界溢�?
slot_ov3:
	ori	$t2, $0, 0x93ac
	lui	$8, 0x7fff
	lui	$9, 0x7fff
	j       slot_ov4
	add     $10, $8, $9		# 测试延迟槽内 add 的下界溢�?
slot_ov4:
	ori     $t2, $0, 0x5daa
	lui	$8, 0x7fff
	lui	$9, 0x8000
	sub	$10, $8, $9			# 测试延迟槽内 sub 的上界溢�?
	lui	$8, 0x8111
	lui	$9, 0x8111
	add     $10, $8, $9
	addi    $10, $8,-2
...
slot_adel2:
	...
	li      $9,0x800
	sw      $9,0($9)        
	lh      $9,0($8)         # 测试lh地址异常—�?�后两位�?1
	j       slot_adel3
	nop
	
slot_adel3:
	li	$8, 0x802
	lw      $9,0($8)         # 测试lw地址异常—�?�后两位�?2
	j       slot_adel4
	nop
	
slot_adel4:
	li	$8, 0x803
	lhu      $9,0($8)        # 测试lhu地址异常—�?�后两位�?3
	j       slot_adel5
	nop
	
slot_adel5:
	li	$8, 0x803
	lw      $9,0($8)         # 测试lw地址异常—�?�后两位�?3
	j       slot_ades1
	nop
	
slot_ades1:
	li	$8, 0x801
	sh      $9,0($8)         # 测试sh地址异常—�?�后两位�?1
	j       slot_ades2
	nop
	
slot_ades2:
	li	$8, 0x802
	sw      $9,0($8)         # 测试sw地址异常—�?�后两位�?2
	j       slot_ades3
	nop
slot_ades3:
	li	$8, 0x801
	sw      $9,0($8)         # 测试lh地址异常—�?�后两位�?1
	j       slot_combination
	nop
	
slot_combination:
	lui      $s0,0x8000      # 异常组合测试，且在连续的异常中插接乘除指令，且涉及了延迟槽中的异常�?�涉及到的冲突有—�?�add 的下界溢�?,addi的上界溢�?,sub的下界溢�?,sw的地�?异常,sh的地�?异常,lw的地�?异常,lh的地�?异常,lhu的地�?异常.
	lui      $s1,0x7fff
	ori      $s1,$s1,0xffff
	add     $10,$s0,$s0
	sub     $10,$s0,$s1
